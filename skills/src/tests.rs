...
fn test_auto_activate_skill() {
    ...
    // Create an AutoActivate skill
    let mut skills = app.world_mut().resource_mut::<Assets<SkillDefinition>>();
    let skill_id = "auto_fire".to_string();
    let skill_handle = skills.add(SkillDefinition {
        id: skill_id.clone(),
        display_name: "Auto Fire".to_string(),
        skill_type: SkillType::AutoActivate,
        cooldown_ms: 1000,
        target: TargetType::Identity,
        effects: vec![SkillEffect::Damage { amount: 10.0 }],
        tags: vec![],
    ...
    // Spawn caster with EquippedSkills
    let _caster = app
        .world_mut()
        .spawn(EquippedSkills(vec![skill_id.clone()]))
        .id();

    // 1. First update: tick_cooldowns should detect missing tracker and trigger
    app.update();
    ...
    assert_eq!(
        query.iter(app.world()).count(),
        1,
        "Should have triggered immediately upon detection"
    );

    // 2. Second update: Should NOT trigger (on cooldown)
    app.update();
    let mut query = app.world_mut().query::<&TestResult>();
    assert_eq!(
        query.iter(app.world()).count(),
        1,
        "Should not trigger again while on cooldown"
    );
    ...
    // 3. Advance time manually
    {
        let mut cooldowns = app
            .world_mut()
            .get_mut::<SkillCooldowns>(_caster)
            .expect("Should have cooldowns");
        let timer = cooldowns
            .timers
            .get_mut(&skill_id)
    ...
    // 4. Update: tick_cooldowns should detect finished timer and trigger again
    app.update();
    ...
    assert_eq!(
        query.iter(app.world()).count(),
        2,
        "Should have triggered again after cooldown finished"
    );
}
