...
use {
    crate::models::{AutopsyFormData, DivinityFormData, ResearchFormData},
    serde::Serialize,
    std::path::Path,
};
...
/// Saves research file to the specified assets directory.
...
/// Saves recipe file to the specified assets directory.
pub fn save_recipe_file(
    data: &crate::models::RecipeFormData,
    assets_dir: &Path,
) -> Result<String, std::io::Error> {
    // Generate content
    // We reuse the generate_recipe_ron logic but need to make sure we have a function for it or use direct RON serialization
    let definition = data.to_recipe_definition();
    let content = ron::ser::to_string_pretty(&definition, ron::ser::PrettyConfig::default())
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;

    // Build paths
    let recipe_dir = assets_dir.join("recipes");

    // Ensure directory exists
    std::fs::create_dir_all(&recipe_dir)?;

    // Build file path
    let recipe_path = recipe_dir.join(data.recipe_filename());

    // Write file
    std::fs::write(&recipe_path, content)?;

    Ok(recipe_path.display().to_string())
}
